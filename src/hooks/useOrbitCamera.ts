...
export function useOrbitCamera() {
    ...
    useFrame(({ camera }, delta) => {
        ...
        // Always face the black hole — the world-space raymarcher reads the
        // real camera matrix, so true lensing follows every angle for free.
        ...
        if (t >= 1.0) {
            ...
            console.log(
                "[Orbit] Complete — handing over to singularity sequence"
            );
        }
    });
}